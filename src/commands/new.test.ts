...
describe('new command', () => {
  ...
  describe('successful worktree creation', () => {
    it('should create worktree with auto-generated name', () => {
      ...
      expect(section).toHaveBeenCalledWith(expect.stringContaining('Creating new worktree'));

      // Should show steps
      expect(step).toHaveBeenCalledWith(1, 2, 'Fetching latest main from origin');
      expect(step).toHaveBeenCalledWith(2, 2, expect.stringContaining('Creating worktree with branch'));
      ...
      expect(indentedSuccess).toHaveBeenCalledWith(expect.stringContaining('Worktree created at'));
      expect(indentedSuccess).toHaveBeenCalledWith(expect.stringContaining('Branch'));
      expect(success).toHaveBeenCalledWith(expect.stringContaining('created successfully'));
      expect(info).toHaveBeenCalledWith(expect.stringContaining('Switch to the worktree'));
    });
    ...
    it('should create worktree with provided name', () => {
      ...
      expect(section).toHaveBeenCalledWith('Creating new worktree "feature-branch"');
      expect(success).toHaveBeenCalledWith('Worktree "feature-branch" created successfully');
    });
    ...
    it('should work with master as default branch', () => {
      ...
      expect(step).toHaveBeenCalledWith(1, 2, 'Fetching latest master from origin');
      expect(createWorktree).toHaveBeenCalledWith(
  ...
  describe('error conditions', () => {
    ...
    it('should handle project config loading failure', () => {
      ...
      expect(() => newCommand()).toThrow('fatal: failed to load project configuration');
    ...
    it('should handle default branch detection failure', () => {
      ...
      expect(() => newCommand()).toThrow('fatal: failed to determine default branch: No default branch found');

      expect(fatal).toHaveBeenCalledWith('failed to determine default branch: No default branch found');
    ...
    it('should handle worktree creation failure', () => {
      ...
      expect(() => newCommand('test-branch')).toThrow('fatal: failed to create worktree: Worktree creation failed');

      expect(fatal).toHaveBeenCalledWith('failed to create worktree: Worktree creation failed');
    });
  });
  ...
  describe('integration with utilities', () => {
    it('should pass correct parameters to utility functions', () => {
      ...
      vi.mocked(getProjectDir).mockReturnValue('/custom/path/projects/my-project');
      vi.mocked(getDefaultBranch).mockReturnValue('main');
      vi.mocked(createWorktree).mockImplementation(() => {});
