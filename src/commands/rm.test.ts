...
const { error, info, success, warning, instruction, section, emphasis, fatal } =
  await import("../output");
...
describe("rm command", () => {
  ...
  describe("successful removal", () => {
    it("should remove a clean worktree", () => {
      ...
      expect(info).toHaveBeenCalledWith(
        "Path: ~/.claudectl/projects/test-project/brave-penguin"
      );
      expect(info).toHaveBeenCalledWith("Last commit: Add user authentication");

      // Should remove the worktree
      expect(removeWorktreeByName).toHaveBeenCalledWith(
        "brave-penguin",
        "test-project",
        tempDir,
        false
      ...
      expect(success).toHaveBeenCalledWith(
        'Session "brave-penguin" removed successfully'
      );
      expect(info).toHaveBeenCalledWith(
        "Removed directory: ~/.claudectl/projects/test-project/brave-penguin"
      );
    ...
    it("should remove a dirty worktree with --force flag", () => {
      ...
      expect(warning).toHaveBeenCalledWith(
        "Forcing removal of session with uncommitted changes"
      );
      ...
      // Should show success message
      expect(success).toHaveBeenCalledWith(
        'Session "swift-fox" removed successfully'
      );
    });
    ...
    it("should handle worktree without commit message", () => {
      ...
      expect(info).not.toHaveBeenCalledWith(
        expect.stringContaining("Last commit:")
      );

      // Should still remove successfully
      expect(success).toHaveBeenCalledWith(
        'Session "test-session" removed successfully'
      );
    });
  });
  ...
  describe("error conditions", () => {
    ...
    it("should exit if current directory is not a git repository", () => {
      ...
      expect(() => rmCommand("test-session")).toThrow(
    ...
    it("should exit if session not found", () => {
      ...
      expect(() => rmCommand("nonexistent-session")).toThrow(
        "process.exit() was called"
      );
      ...
      expect(info).toHaveBeenCalledWith(
        "List available sessions with: claudectl list"
      );
      expect(processExitSpy).toHaveBeenCalledWith(1);
    });
    ...
    it("should exit if session has uncommitted changes without --force", () => {
      ...
      expect(() => rmCommand("dirty-session")).toThrow(
        "process.exit() was called"
      );
    ...
    it("should exit if trying to remove current worktree", () => {
      ...
      expect(() => rmCommand("current-session")).toThrow(
        "process.exit() was called"
      );
    ...
    it("should handle worktree removal failure", () => {
      ...
      expect(() => rmCommand("test-session")).toThrow(
        "fatal: failed to remove session: Git command failed"
      );

      expect(fatal).toHaveBeenCalledWith(
        "failed to remove session: Git command failed"
      );
    });
  });
  ...
  describe("integration with utilities", () => {
    it("should pass correct parameters to utility functions", () => {
      ...
      expect(findWorktreeByName).toHaveBeenCalledWith(
        "test-session",
        "my-project",
        tempDir
      );
