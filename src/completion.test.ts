...
const { hasClaudectlConfig, loadProjectConfig, getProjectWorktrees, getWorktreeName } = await import('./utils');
const { handleCompletion, installCompletion, uninstallCompletion } = await import('./completion');
...
describe('completion', () => {
  ...
  describe('handleCompletion', () => {
    ...
    it('should complete main commands for length 1', async () => {
      ...
      expect(tabtab.default.log).toHaveBeenCalledWith([
        'init', 'new', 'list', 'rm', 'install-completion', 'uninstall-completion'
      ]);
    });
    ...
    it('should filter completions based on partial input', async () => {
      ...
      expect(tabtab.default.log).toHaveBeenCalledWith(['init', 'install-completion']);
    });
  ...
  describe('installCompletion', () => {
    it('should install completion successfully', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith('✓ Tab completion installed successfully!');
      expect(mockConsoleLog).toHaveBeenCalledWith('  Restart your shell or run: source ~/.bashrc (or ~/.zshrc)');
    ...
    it('should handle installation failure', async () => {
      ...
      await expect(installCompletion()).rejects.toThrow('process.exit() was called');

      expect(mockConsoleError).toHaveBeenCalledWith('✗ Failed to install tab completion:', error);
  ...
  describe('uninstallCompletion', () => {
    it('should uninstall completion successfully', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith('✓ Tab completion uninstalled successfully!');
    ...
    it('should handle uninstallation failure', async () => {
      ...
      await expect(uninstallCompletion()).rejects.toThrow('process.exit() was called');

      expect(mockConsoleError).toHaveBeenCalledWith('✗ Failed to uninstall tab completion:', error);
      expect(mockProcessExit).toHaveBeenCalledWith(1);
