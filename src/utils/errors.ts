import { error, fatal, instruction } from "../output.js";
import {
  hasClaudectlConfig,
  isGitRepository,
  loadProjectConfig,
 ...
 */
export function handleProjectValidation(
  currentDir: string,
  projectName?: string
): { name: string } {
  ...
  if (!isGitRepository(currentDir)) {
    ...
    instruction(
      ...
      [
        ...
        projectName ? `claudectl ${projectName}` : "claudectl list",
      ]
    );
    process.exit(1);
  }
  ...
  if (!hasClaudectlConfig(currentDir)) {
    ...
    instruction("Please initialize a claudectl project first:", [
      "claudectl init",
    ]);
    process.exit(1);
  }
...
export function validateSessionName(
  ...
  if (!sessionName || sessionName.trim().length === 0) {
    ...
    instruction(`Specify the name of the session to ${operation}:`, [
      `claudectl ${operation} brave-penguin`,
      `claudectl ${operation} swift-fox`,
    ]);
    process.exit(1);
  }
}
